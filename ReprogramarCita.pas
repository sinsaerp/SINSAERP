...
begin
  MessageDlg('¿Está seguro que desea reprogramar la cita seleccionada?',
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            ShowMessage
              ('Ya existe un paciente para la fecha y hora seleccionada');
            exit;
          end
          else
          begin
            ...
            UniMainModule.Query.SQL.Clear;
            UniMainModule.Query.SQL.Text :=
              'UPDATE citas set horacita=:Hor, horacitax=:Horx , hora=:Hora where consecutivo=:Con';
            UniMainModule.Query.ParamByName('Horx').Value := Hora.DateTime;
            UniMainModule.Query.ParamByName('Hora').Value := Hora.DateTime;
            UniMainModule.Query.ParamByName('Hor').Value :=
              FormatDateTime('hh:mm AM/PM', Hora.DateTime);
            ...
            UniMainModule.Query.SQL.Clear;
            UniMainModule.Query.SQL.Add
              ('Delete From agendatemp where medico =''' +
              CodMedico.Text + '''');
            UniMainModule.Query.ExecSQL;
