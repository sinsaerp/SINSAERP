  ...
  begin
    ...
    if Assigned(ClientList) then
      try
        ...
        ClientList.Free;
        UniMainModule.Query.SQL.Clear;
        cadena := 'select c.medico as codmedico, c.codeps, c.id, c.afape1, c.afape2, c.afnom1, c.afnom2, c.nombrec, c.tipidafil, c.afcodigo, c.sexo, c.telefono, c.email, c.fecha_nacimiento,  c.direccion, c.idapi, c.fechaCita, c.fechaSolicitud,'
...
begin
  ...
  begin
    ...
    begin
      if Turnos.Text = '' then
      begin
        ShowMessage('Turnos en la mañana esta vacio');
        exit;
      end;
      ...
      begin
        ShowMessage
          ('Los turnos web en la jornada mañana es mayor a los turnos presenciales');
        exit;
      end;
...
begin
  MessageDlg
    ('¿Está seguro que desea eliminar este registro de horario del médicos?',
